...
void update_ble_sensor_characteristics() {
    SensorData sensor_data;

    sensor_data = read_sensor_data(&dev);
    // Convert sensor data to BLE-compatible format (byte arrays)
    uint8_t temp_data[4];
    uint8_t humidity_data[4];

    // Simple conversion, assuming little-endian architecture
    memcpy(temp_data, &sensor_data.temperature, sizeof(sensor_data.temperature));
    memcpy(humidity_data, &sensor_data.humidity, sizeof(sensor_data.humidity));

    // Prepare handle-value pairs
    cy_stc_ble_gatt_handle_value_pair_t tempHandleValuePair = {
        .attrHandle = CY_BLE_ENVIRONMENTAL_TEMPERATURE_CHAR_HANDLE,
        .value = { temp_data, sizeof(temp_data) }
    };

    cy_stc_ble_gatt_handle_value_pair_t humidityHandleValuePair = {
        .attrHandle = CY_BLE_ENVIRONMENTAL_HUMIDITY_CHAR_HANDLE,
        .value = { humidity_data, sizeof(humidity_data) }
    };

    // Write attribute values to the BLE stack
    Cy_BLE_GATTS_WriteAttributeValuePeer(&app_conn_handle, &tempHandleValuePair);
    Cy_BLE_GATTS_WriteAttributeValuePeer(&app_conn_handle, &humidityHandleValuePair);
}

void update_ble_soil_characteristics(){
    float data;
    data = read_soil_moisture();
    uint8_t soil_data[4];
    // float soil_data[4];
    memcpy(soil_data, &data, sizeof(data));

    cy_stc_ble_gatt_handle_value_pair_t soilHandleValuePair = {
        .attrHandle = CY_BLE_ENVIRONMENTAL_MOISTURE_CHAR_HANDLE,
        .value = { soil_data, sizeof(data) }
    };

    Cy_BLE_GATTS_WriteAttributeValuePeer(&app_conn_handle, &soilHandleValuePair);
}
...
void ble_findme_process(void)
{
    ...
    if(wakeup_intr_flag)
    {
        ...
        update_ble_sensor_characteristics();
        update_ble_soil_characteristics();
...
static void stack_event_handler(uint32_t event, void* eventParam)
{
    switch(event)
    {
        ...
        case CY_BLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ:
        {
            ...
            SensorData sensor = read_sensor_data(&dev);

            if (CY_BLE_ENVIRONMENTAL_TEMPERATURE_CHAR_HANDLE == read_req_param->attrHandle)
            {
            	printf("[INFO] : trying to read from temperature characteristic\r\n");

            	CY_BLE_GATT_DB_ATTR_SET_GEN_VALUE(CY_BLE_ENVIRONMENTAL_TEMPERATURE_CHAR_HANDLE,&sensor.temperature,sizeof(sensor.temperature));

                printf("[INFO] : temperature %ld.%02lu °C\n\r", sensor.temperature / 100, sensor.temperature % 100);
            }

            if (CY_BLE_ENVIRONMENTAL_HUMIDITY_CHAR_HANDLE == read_req_param->attrHandle)
            {
            	printf("[INFO] : trying to read from Humidity characteristic\r\n");
                SensorData sensor = read_sensor_data(&dev);
            	CY_BLE_GATT_DB_ATTR_SET_GEN_VALUE(CY_BLE_ENVIRONMENTAL_HUMIDITY_CHAR_HANDLE, &sensor.humidity, sizeof(sensor.humidity));

                printf("[INFO] : Humidity  %lu%%\n\r", sensor.humidity);
            }

            float data = read_soil_moisture();

            if (CY_BLE_ENVIRONMENTAL_MOISTURE_CHAR_HANDLE == read_req_param->attrHandle)
            {
            	printf("[INFO] : trying to read from soil moisture characteristic\r\n");
                
            	CY_BLE_GATT_DB_ATTR_SET_GEN_VALUE(CY_BLE_ENVIRONMENTAL_MOISTURE_CHAR_HANDLE, &data, sizeof(data));

                printf("[INFO] : Soil Moisture %f\n\r", data);
            }
        ...
        case CY_BLE_EVT_GATTS_WRITE_CMD_REQ:
        {
            while(1){
                ...
                if(CY_BLE_CONTROL_ALL_CONTROL_CHAR_HANDLE == write_req_param->handleValPair.attrHandle){
                
                    switch (write_req_param->handleValPair.value.val[0]) {
                        // Grow Light Control
                        case 0: 
                            switch_relay1_off();
                            break;
                        ...
                        // Catch-all
                        default:
                            printf("Invalid write command number used \r\n");
                            break;
                    }
                    printf("[INFO] : GATT write control characteristic with value: 0x%x\r\n", write_req_param->handleValPair.value.val[0]);
                    break;
                }
